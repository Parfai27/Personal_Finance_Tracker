...
function renderBalanceTrendChart() {
    ...
    sortedTransactions.forEach(t => {
        const date = t.date?.toDate ? t.date.toDate() : new Date(t.dateString || t.date);
        ...
        dateData[dateKey] = runningBalance;
    });
    ...
    transactions.forEach(t => {
        if (t.type === 'income') {
            if (!incomeCategories[t.category]) {
                incomeCategories[t.category] = { count: 0, total: 0 };
            }
            incomeCategories[t.category].count++;
            incomeCategories[t.category].total += parseFloat(t.amount);
        } else {
            if (!expenseCategories[t.category]) {
                expenseCategories[t.category] = { count: 0, total: 0 };
            }
            expenseCategories[t.category].count++;
            expenseCategories[t.category].total += parseFloat(t.amount);
        }
    });

    // Render income categories
    renderCategoryList('income-categories-list', incomeCategories, 'income');
    renderCategoryList('expense-categories-list', expenseCategories, 'expense');

    // Update counts
    const incomeCountEl = document.getElementById('income-cat-count');
    if (incomeCountEl) incomeCountEl.textContent = `(${Object.keys(incomeCategories).length})`;

    const expenseCountEl = document.getElementById('expense-cat-count');
    if (expenseCountEl) expenseCountEl.textContent = `(${Object.keys(expenseCategories).length})`;
}
...
function renderCategoryList(containerId, categories, type) {
    ...
    if (Object.keys(categories).length === 0) {
        container.innerHTML = `<p class="text-center" style="padding: 2rem; color: var(--text-light); grid-column: 1 / -1;">No ${type} categories found.</p>`;
        return;
    }
    ...
    const badgeColor = isIncome ? 'success-badge' : 'danger-badge';
        ...
        const categoryCard = `
            <div class="category-card">
                <div class="category-card-header">
                    <div class="category-icon" style="background-color: ${bgColor}; color: ${textColor}">
                        ${icon}
                    </div>
                ...
                <div class="category-card-footer">
                    <div class="category-total" style="color: ${textColor}">
